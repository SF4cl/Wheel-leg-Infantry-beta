...
extern  uint32_t rx_len; //接收数据的长度
extern  uint32_t recv_end_flag; //接收结束标志位
uint32_t usart5_rx_len;
...
void UART5_IRQHandler(void)
{
  ...
  uint8_t tmp_flag =__HAL_UART_GET_FLAG(&huart5,UART_FLAG_IDLE); //获取IDLE状态	
  if((tmp_flag != RESET))//判断接收是否结束
    { 
			  ...
			  //HAL_UART_DMAStop(&huart5); 
				...
				if(usart5_rx_len>1)
				{
						if(usart5_flag==1)
						{
								...
								SCB_InvalidateDCache_by_Addr(usart5_buf1,usart5_rx_len);
								fifo_s_puts(&referee_fifo,(char*)usart5_buf1, usart5_rx_len);							
								HAL_UART_Receive_DMA(&huart5,usart5_buf2,USART_RX_BUF_LENGHT);//开启DMA
								usart5_flag=2;
						}
						else
						{
								SCB_InvalidateDCache_by_Addr(usart5_buf2,usart5_rx_len);
								fifo_s_puts(&referee_fifo,(char*)usart5_buf2, usart5_rx_len);							
								HAL_UART_Receive_DMA(&huart5,usart5_buf1,USART_RX_BUF_LENGHT);//开启DMA	
								usart5_flag=1;
						}
				}
		}	
